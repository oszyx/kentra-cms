layui.use(['form','layer','jquery','laydate'], function () {
    ...
    //显示维修设备信息
    form.val("frameRepair", {
    	"repairDeviceId":frameData.id,
        "repairDeviceName": frameData.name
    });
    ...
    form.verify({
    	...
    	repairDateEnd:function(value){
    		...
    		if (starTime!=null && starTime !='' && new Date(starTime) > new Date(value)) {
                return '维修结束日期不能小于开始时间';
            }
    	}
    });
    //提交维修信息
    form.on('submit(frameRepairSubmitBtn)',function(data){
    	var datas=data.field;
    	$.ajax({
        	url: '/equip/frame/insertRepair',
        	type: "post",
        	data: data.field,
        	dataType: "json",
        	async:false,
