layui.config({
    ...
    table.render({
        ...
        , where: {customerStatus: "customer_status_1"}
        , id: 'blacklistTable'
        , cols: [[ //表头
            ...
            /*, {fixed: 'right', width: 165, align: 'center', title: '操作', toolbar: '#blacklistTableBar'}*/
        ]]
    });

    //监听工具条
    table.on('tool(blacklistTableFilter)', function (obj) { //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
    ...
    $("#deleteBtn").click(function () {
        //获得表格选中行Id
        var ids = active.getCheckAttrValue('blacklistTable', 'id');
        var data = active.getCheckData('blacklistTable');
        if (data.length > 0) {
            parent.layer.confirm('确定要将该用户移除黑名单？', {icon: 3}, function (index) {
                    ...
                    success: function (result) {
                        if (result != "" && result != null) {
                            if (result.statusCode == "200") {
                                active.reLoadTableData('blacklistTable', '/customer/management/customer/getPageList');
                                parent.layer.msg('移除黑名单成功', {icon: 1, shade: 0.4, time: 1000})
                                parent.layer.close(index);
                            } else if (result.statusCode == "300") {
                                parent.layer.msg("移除黑名单失败", {icon: 5, shade: [0.4], time: 1000});
                            }
                        }
                    },
    ...
    $("#printBtn").click(function () {
        var formData = active.getCheckData('blacklistTable');
        if (formData.length < 1) {
            ...
            for (var i = 0; i < drivingLicenseList.length; i++) {
                ...
                var rowTem =
                    ...
                    "</td></tr>";
                $("#blackPrintTbody").append(rowTem);
            }
            //打印
            $("#blackPrint").jqprint({debug: false, importCSS: true, printContainer: true, operaSupport: false});
            $(".cleanDate").empty();
            $("#blackPrintTbody").empty();
        }
    });
    ...
    $("#exportBtn").click(function () {
        parent.layer.confirm('确定要导出客户信息？', {icon: 3}, function () {
            parent.layer.closeAll('dialog');
            // 查询内容
            // var phoneNum = $("#searchParam").val();
            window.location.href = '/customer/management/customer/exportCustomerInfos'
                + '?phoneNum=' + searchPhoneValue + '&name=' + searchNameValue + '&driveNum=' + searchDrivNumValue + '&code=customer_status_1';
        })
    });
});